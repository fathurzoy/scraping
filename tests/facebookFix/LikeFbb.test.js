	...
	try {
		...
		if (elementToInteract > 0) {
			...
			if (elementToLike.length > 0) {
				boundingBox = await elementToLike[0].boundingBox() // mendapatkan kotak batas di sekitar elemen
				await elementToLike[0].click()
				console.log('LIKED!')
				await page.waitForTimeout(1000)
			} else {
				console.log('No element to like found')
			}

			const timestamp = Date.now() // mengambil timestamp saat ini
			const screenshotPath = `./tests/e2e/screenshootFb/likeFb-${timestamp}.png` // menggabungkan timestamp ke dalam nama file

			if (boundingBox) {
				// hanya jika variabel boundingBox telah ditetapkan nilai
				// mengambil tangkapan layar hanya pada area elemen yang di-klik
				await page.screenshot({
					path: screenshotPath, // gunakan path dengan timestamp
					clip: {
						x: boundingBox.x,
						y: boundingBox.y,
